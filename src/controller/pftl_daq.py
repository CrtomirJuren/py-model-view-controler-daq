...
- Only pure hardware functions and in/out limit cheching
- Outputs 0...4095
- input   0...1023
"""
...
#TODO preimenuj to da bodo DAQ classi smiselni. Npr DAQParent, DAQSim, DAQPftl, DAQNIUSB6000
class Device:
    """ """
    DEFAULTS = {
        'baudrate': 9600,
    ...
    def set_analog_output(self, channel:int, output_value:int) -> None:
        ...
        # 1st check for TypeError
        if output_value != int(output_value):
            raise TypeError("protected_value must be an integer")
        else:
            #2nd check for ValueError
            if low_limit <= output_value <= high_limit:
                # self._protected_value = int(output_value)
                msg_tx = f'OUT:CH{channel}:{output_value}\n'
                self.query(msg_tx)
            else:
                raise ValueError(f"set_analog_output ValueError: low limit = {low_limit}, high_limit = {high_limit}")
